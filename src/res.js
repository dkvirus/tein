...
function traverse2Value(response) {
  ...
  for (let attr in response) {
    ...
    if (
      ...
      response[attr] === Number ||
      (typeof response[attr] === "string" && response[attr] === "Number") ||
      typeof response[attr] === "number"
    ) {
      obj[attr] = 0;
    } else if (
      response[attr] === Boolean ||
      (response[attr] === "Boolean" && typeof response[attr] === "string") ||
      typeof response[attr] === "boolean"
    ) {
      obj[attr] = false;
    }
  }
...
function handleObject(response, expect) {
  ...
  if (JSON.stringify(respResult) !== JSON.stringify(expeResult)) {
    console.log(
      clc.red(`>【${req.method} ${req.url}】接口返回数据结构有变化：`)
    );
    console.log(
      differ(
        JSON.stringify(expeResult, null, 4),
        JSON.stringify(respResult, null, 4)
      )
    );
